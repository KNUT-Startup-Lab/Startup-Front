...
export default function SignupStudent() {
  ...
  // ⬇ 단과대/전공 제거: 이름, 학번, 이메일, 비밀번호, 전화번호만 사용
  ...
  const set = <K extends keyof SignupReq>(k: K, v: string) =>
    setForm(prev => ({ ...prev, [k]: v }));

  async function onSubmit() {
    try {
      if (!form.name || !form.student_num || !form.email || !form.password) {
        return Alert.alert('안내', '이름/학번/이메일/비밀번호를 입력하세요.');
      }
      const chk = await AuthAPI.checkEmail(form.email);
      if (!chk.available) {
        return Alert.alert('안내', chk.message || '이미 사용 중인 이메일입니다.');
      }
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>학생 회원 가입</Text>
